    ...
    switch Params.Model
       ...
       case {"biexp_T1corr","Biexp_T1corr"}
        ...
        case {"triexp","Triexp"}
            ...
            % f_fast
            op_out.Lower(2) = fit_res.f_fast(x,y)*(1-Params.Tol(2));
            op_out.StartPoint(2) = fit_res.f_fast(x,y);
            op_out.Upper(2) = fit_res.f_fast(x,y)*(1+Params.Tol(2));

            % D_slow
            op_out.Lower(3) = fit_res.D_slow(x,y)*(1-Params.Tol(3));
            op_out.StartPoint(3) = fit_res.D_slow(x,y);
            op_out.Upper(3) = fit_res.D_slow(x,y)*(1+Params.Tol(3));

            % D_inter
            op_out.Lower(4) = fit_res.D_inter(x,y)*(1-Params.Tol(4));
            op_out.StartPoint(4) = fit_res.D_inter(x,y);
            op_out.Upper(4) = fit_res.D_inter(x,y)*(1+Params.Tol(4));

            % D_fast
            op_out.Lower(5) = fit_res.D_fast(x,y)*(1-Params.Tol(5));
            op_out.StartPoint(5) = fit_res.D_fast(x,y);
            op_out.Upper(5) = fit_res.D_fast(x,y)*(1+Params.Tol(5));
